...
def get_links(link):
    ...
    try:
        ...
        print("Ошибка связанная с попыткой напичатать available_links или page_links")
...
def write_content(link):
    ...
    for i in range(len(links)):
        try:
            ...
            time.sleep(5)
            continue
        with open(f'Выкачка/{i + 1}.txt', 'w', encoding='utf-8') as out, open('index.txt', 'a', encoding='utf-8') as file:
            out.write(str(soup.text.strip()))
            file.write(f'{i + 1} {links[i]}\n')
